...
### Figure 2 
T150 <- survfit(Surv(value)~threshold, data=mdata2[mdata2$threshold==-150, ])
T250 <- survfit(Surv(value)~threshold, data=mdata2[mdata2$threshold==-250, ])
par(mfrow=c(1, 2))
  plot(T150, log="xy", conf.int=FALSE, xlab="inter-occurrence time t (days)", ylab="S(t)", main="b. Dst < -250nT")
  lines(predict(reg.weibull, newdata=list(threshold=-150),type="quantile",p=seq(0,1,by=.0001)),seq(1,0,by=-.0001),col="red")
  lines(predict(reg.expo, newdata=list(threshold=-150),type="quantile",p=seq(0,1,by=.0001)),seq(1,0,by=-.0001),col="blue")
  plot(T250, log="xy", conf.int=FALSE, xlab="inter-occurrence time t (days)", ylab="S(t)", main="b. Dst < -250nT")
  lines(predict(reg.weibull, newdata=list(threshold=-250),type="quantile",p=seq(0,1,by=.0001)),seq(1,0,by=-.0001),col="red")
  lines(predict(reg.expo, newdata=list(threshold=-250),type="quantile",p=seq(0,1,by=.0001)),seq(1,0,by=-.0001),col="blue")
dev.off()
...
load("Scientific Reports/Supplementary Material/difftimes_NoMAX.RData")
mdata <- melt(diff.times)
for (i in 1:40)
{
  eval(parse(text=paste0("mdata$threshold[mdata$variable=='diff", i, "'] <- -10*i")))
}
...
AIC(reg.weibull); AIC(reg.gamma) ### AIC corresponding to the Weibull model is lower, so this is the preferred model
...
ggplot(dst3, aes(x=DST)) + geom_histogram(binwidth=0.1) + 
  scale_x_continuous(breaks=c(0, 25, 50, 100, 150, 200), trans="log1p", expand=c(0, 0)) +
  scale_y_continuous(breaks=c(0, 5000, 10000, 20000, 50000, 100000), expand=c(0, 0)) + xlab("-DST") +
  theme_bw()

ggplot(dst3, aes(x=DST)) +
